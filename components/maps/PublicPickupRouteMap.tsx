...
export default function PublicPickupRouteMap({ route }: { route: PublicRoute }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (points.length > 1) L.polyline(points, { color: "#dc2626", weight: 5, opacity: .82, dashArray: "10 8" }).addTo(map);
  ...
  if (!cities.length) return (
    <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[2rem] border border-slate-200 bg-slate-50 p-8 text-center">
  ...
  return (
    <div className="relative min-h-[420px] overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-100">
      <div ref={ref} className="h-[420px] w-full" aria-label={`Mapa público de ${route.name}`} />
      ...
      <style jsx global>{`
        ...
        .public-route-marker--start span{background:#071d43}
