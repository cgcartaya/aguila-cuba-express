...
import { Crosshair, Loader2, LocateFixed, RotateCcw } from "lucide-react";
...
export default function LocationPickerMap({ latitude, longitude, onChange, onClear, enableGeolocation = false }: Props) {
  ...
  function placeMarker(lat: number, lng: number, zoom = 16) {
  ...
  useEffect(() => {
    ...
    async function initialize() {
      ...
      const map = L.map(containerRef.current).setView(initial, latitude != null ? 16 : 7);
      mapRef.current = map;
      ...
      map.on("click", (event: any) => {
        placeMarker(event.latlng.lat, event.latlng.lng, map.getZoom());
  ...
  function useCurrentLocation() {
    ...
    if (!navigator.geolocation) {
      setGeoError("Este dispositivo no permite obtener la ubicación.");
      return;
    }
    setLocating(true);
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setGeoError("No pudimos obtener tu ubicación. Revisa el permiso del navegador.");
        setLocating(false);
      },
  ...
  return (
    <div className="space-y-3">
      <p className="flex items-start gap-2 text-sm font-bold text-slate-600">
        <Crosshair className="mt-0.5 shrink-0 text-blue-600" size={17} />
        Toca el punto exacto o arrastra el marcador hasta la entrada correcta.
      </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <span className="text-xs font-bold text-slate-500">
          {latitude != null && longitude != null ? `${latitude.toFixed(6)}, ${longitude.toFixed(6)}` : "Ubicación todavía no seleccionada"}
        </span>
        <div className="flex flex-wrap gap-2">
          {enableGeolocation && (
            <button type="button" onClick={useCurrentLocation} disabled={locating} className="rounded-xl bg-[#082b5c] px-3 py-2 text-sm font-black text-white disabled:opacity-60">
              {locating ? <Loader2 size={15} className="mr-1 inline animate-spin" /> : <LocateFixed size={15} className="mr-1 inline" />}
              Usar mi ubicación
            </button>
          )}
          ...
          {latitude != null && <button type="button" onClick={onClear} className="rounded-xl border px-3 py-2 text-sm font-black text-red-600"><RotateCcw size={15} className="mr-1 inline" />Quitar punto</button>}
        </div>
      </div>
