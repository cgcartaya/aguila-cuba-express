...
export default function MenuPageClient({
  ...
  const hasBar = availableCategories.some(
    (category) =>
      category.venue_type === "bar" ||
      category.venue_type === "general"
  );
  ...
  useEffect(() => {
    fetch(
      `/api/public/menu-availability?slug=${encodeURIComponent(
        storeSlug
      )}`
    )
    ...
    >
      ...
      {activeItem && !editing && (
        <MenuItemModal
          ...
          onClose={() => {
            setActiveItem(null);
            if (totalItems > 0) setCartOpen(true);
          }}
          onAdd={(line) => {
            addFromModal(line);
            setActiveItem(null);
