...
export default function MenuDistanceDeliveryFields({ storeId, address, origin, quote, onAddressChange, onQuote }:
  ...
  return <div className="space-y-3">
    ...
    {quote && !quoting && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-bold text-emerald-900"><p className="flex items-center gap-2 font-black"><Truck size={16} /> Dirección confirmada</p><p className="mt-1">{quote.zone ? `${quote.zone} · ` : ""}Distancia: {(quote.distanceMeters / 1000).toFixed(2)} km · Entrega: ${quote.fee.toFixed(2)} CUP</p>{quote.catalogId && <button type="button" onClick={() => { onQuote(null); setShowMap(true); }} className="mt-2 font-black underline">No es esta dirección · marcar en mapa</button>}</div>}
