      ...
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl bg-white sm:rounded-3xl">
          ...
          {cart.length === 0 ? (
            ...
            <div className="space-y-3">
              {cart.map((line) => {
                ...
                return (
                  <div key={line.lineId} className="rounded-2xl bg-slate-50 p-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-sm font-black text-slate-900">{line.name}</p>
                        {line.selected_options.map((opt) => (
                          <p key={opt.option_id} className="text-xs font-semibold text-slate-500">
        ...
        {cart.length > 0 && (
          <div className="space-y-3 border-t border-slate-100 p-5">
            ...
            <button
              onClick={handleSend}
              className="flex w-full items-center justify-center gap-2 rounded-full px-5 py-3.5 text-sm font-black text-white shadow-sm transition"
