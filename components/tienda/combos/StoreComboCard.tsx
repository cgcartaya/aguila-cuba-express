...
export default function StoreComboCard({ combo }: Props) {
  ...
  return (
    <article className="flex h-full w-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
      <div className="flex flex-1 gap-4 p-4">
        <Link
          href={`/tienda/combos/${combo.id}`}
          className="relative h-[150px] w-[42%] shrink-0 overflow-hidden rounded-xl bg-white"
        >
          {savings > 0 && (
      ...
      <button
        ...
        className="w-full bg-red-600 py-3 text-sm font-black text-white transition hover:bg-red-700"
      >
        Agregar combo
      </button>
    </article>
