...
const getCachedQuoteConfig = unstable_cache(
  async (keyType: "subdomain" | "domain", keyValue: string) => {
    ...
    storeQuery = keyType === "subdomain"
      ? storeQuery.eq("subdomain", keyValue)
      : storeQuery.eq("domain", keyValue);
    ...
    if (queryError) {
      ...
      return { status: 500, body: { error: "No se pudo cargar la configuración del cotizador." } };
    }

    const settings = settingsResult.data;
    if (!settings?.quote_enabled) {
      return { status: 404, body: { error: "El cotizador no está disponible." } };
    }
  ...
  ["public-quote-config-v2"],
  { revalidate: 300, tags: ["public-quote-config"] },
);

export async function GET(request: NextRequest) {
  const key = getStoreKey(request);
  const result = await getCachedQuoteConfig(key.type, key.value);
