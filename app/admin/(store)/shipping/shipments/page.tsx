...
export default function ShippingShipmentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return shipments.filter((s) =>
      ...
      (!query ||
        String((s as Shipment & { order_number?: number | null }).order_number || "").includes(query) ||
    ...
    <main className="min-h-screen bg-slate-50 p-4 pb-24 md:p-6">
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div><p className="text-sm font-black text-blue-700">Operaciones</p><h1 className="text-3xl font-black text-[#061b3a] md:text-4xl">Lista de envíos</h1><p className="mt-1 text-sm font-semibold text-slate-500">{activeStore?.name}</p></div>
          <div className="flex flex-wrap gap-2">
            <Link href="/admin/shipping" className="inline-flex items-center justify-center gap-2 rounded-2xl border bg-white px-5 py-3 text-sm font-black text-[#061b3a]">Dashboard</Link>
        ...
        <section className="mb-5 rounded-3xl border bg-white p-4 shadow-sm">
          <div className="grid gap-3 md:grid-cols-[1fr_240px]">
            <label className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={19}/><input value={search} onChange={(e)=>setSearch(e.target.value)} className="w-full rounded-2xl border py-3 pl-12 pr-4 text-sm font-semibold" placeholder="Número, código, nombre, teléfono o lugar"/></label>
