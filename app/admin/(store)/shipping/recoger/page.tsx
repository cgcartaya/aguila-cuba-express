...
export default function PickupShipmentPage() {
  ...
  const [shipment, setShipment] = useState<{ id: string; trackingCode: string; servicePrice: number } | null>(null);
  ...
  // Si venimos de vuelta del Checkout de Stripe, consultamos el estado real
  // del envío (el webhook puede tardar un instante en llegar).
  useEffect(() => {
    async function checkReturn() {
      ...
      // El webhook normalmente ya llegó, pero por si acaso reintentamos
      // un par de veces con una pequeña espera.
      for (let attempt = 0; attempt < 5; attempt++) {
        ...
        if (json.paymentStatus === "paid") {
          setShipment({ id: returningShipmentId, trackingCode: json.trackingCode, servicePrice: Number(json.servicePrice || 0) });
  ...
  async function submit(input: ShipmentInput) {
    ...
    // el único abierto, o el más reciente — igual que hacía la pantalla
    // completa antes de que el usuario tocara nada.
        ...
        {step === "payment" && shipment && (
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-10">
            <div className="text-center">
              ...
              <p className="mt-2 font-semibold text-slate-500">Elige cómo te está pagando el cliente ahora mismo.</p>
            </div>
            ...
            <div className="mx-auto mt-8 grid max-w-xl gap-4 sm:grid-cols-2">
              ...
              >
                {payBusy === "card" ? <Loader2 size={40} className="animate-spin" /> : <CreditCard size={40} />}
