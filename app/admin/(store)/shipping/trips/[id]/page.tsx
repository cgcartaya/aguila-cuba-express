...
export default function ShippingTripDetailPage() {
  ...
  async function changeOneStatus(shipment: Shipment, status: ShippingStatus) {
    ...
    if (!window.confirm(`¿Cambiar la parada #${shipment.trip_order || "—"} a “${getShippingStatusLabel(status)}”?`)) return;
    setWorking(true);
  ...
  async function shareManifestPdf() {
    if (!trip || selectedShipments.length === 0) return;
    const manifestTrip = trip;
    setWorking(true);
    setError("");

    try {
      // Se carga solamente al pulsar Compartir PDF. Así la librería no forma
      // parte del JavaScript inicial de la pantalla del viaje.
      ...
      function drawHeader() {
        pdf.setFillColor(6, 27, 58);
        pdf.rect(0, 0, pageWidth, 31, "F");
        pdf.setTextColor(255, 255, 255);
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(17);
        pdf.text(`Manifiesto de viaje ${manifestTrip.trip_number}`, margin, 11);
        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(9);
        pdf.text(`${manifestTrip.name} | ${manifestTrip.origin || "Origen sin definir"} -> ${manifestTrip.destination || "Destino sin definir"}`, margin, 18);
        const summary = [`Envios: ${selectedShipments.length}`];
        if (totalWeight > 0) summary.push(`Peso: ${totalWeight.toFixed(1)} lb`);
        if (totalMoney > 0) summary.push(`Dinero: ${money(totalMoney)}`);
        pdf.text(summary.join("   |   "), margin, 25);
        pdf.setTextColor(15, 23, 42);
      }

      function drawColumns(y: number) {
        pdf.setFillColor(226, 232, 240);
        pdf.rect(margin, y, pageWidth - margin * 2, 8, "F");
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(7.5);
        pdf.text("Parada / rastreo", 12, y + 5);
        pdf.text("Remitente", 47, y + 5);
        pdf.text("Destinatario", 94, y + 5);
        pdf.text("Destino", 141, y + 5);
        pdf.text("Cantidad / estado", 202, y + 5);
        pdf.text("Repartidor", 252, y + 5);
      }
      ...
      for (const shipment of selectedShipments) {
        ...
        const cells = [
          pdf.splitTextToSize(`#${shipment.trip_order || "-"}\n${shipment.tracking_code || shipment.id.slice(0, 8)}\n${shipmentTypeLabel(shipment)}`, 31),
          pdf.splitTextToSize(`${shipment.sender_name || "Sin cliente"}\n${shipment.sender_phone || ""}`, 43),
          pdf.splitTextToSize(`${shipment.recipient_name || "Sin destinatario"}\n${shipment.recipient_phone || ""}`, 43),
          pdf.splitTextToSize(`${shipment.location || "Sin lugar"}\n${shipment.recipient_address || ""}`, 57),
          pdf.splitTextToSize(`${cantidadParts.join(" + ") || "-"}\n${getShippingStatusLabel(shipment.status)}`, 46),
          pdf.splitTextToSize(shipment.assigned_driver_name || "Sin asignar", 32),
        ];
        const rowHeight = Math.max(14, Math.max(...cells.map((cell) => cell.length)) * 4 + 3);
        ...
        pdf.rect(margin, y - 2, pageWidth - margin * 2, rowHeight, "S");
        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(7.5);
        [12, 47, 94, 141, 202, 252].forEach((x, index) => pdf.text(cells[index], x, y + 2));
        y += rowHeight;
      }
      ...
      for (let page = 1; page <= pages; page += 1) {
        ...
        pdf.text(`Generado ${new Date().toLocaleString("es-US")} | Pagina ${page} de ${pages}`, pageWidth / 2, pageHeight - 5, { align: "center" });
      }

      const blob = pdf.output("blob");
      const safeTrip = String(manifestTrip.trip_number).replace(/[^a-zA-Z0-9_-]/g, "-");
      const fileName = `manifiesto-viaje-${safeTrip}.pdf`;
      const file = new File([blob], fileName, { type: "application/pdf" });
      const shareData = { title: `Manifiesto viaje ${manifestTrip.trip_number}`, text: `${selectedShipments.length} envios`, files: [file] };
      ...
      if (navigator.share && (!navigator.canShare || navigator.canShare(shareData))) {
        ...
        window.alert("El navegador no permite compartir archivos directamente. El PDF completo fue descargado; adjuntalo en WhatsApp.");
      }
    } catch (shareError) {
      if (shareError instanceof DOMException && shareError.name === "AbortError") return;
      console.error("No se pudo generar o compartir el manifiesto:", shareError);
      setError("No se pudo generar el PDF del manifiesto.");
    } finally {
      setWorking(false);
    }
  }
