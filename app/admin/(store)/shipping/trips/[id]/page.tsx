...
export default function ShippingTripDetailPage() {
  ...
  const tripSequenceById = useMemo(() => {
    const ordered = [...shipments].sort((a, b) => {
      const orderA = Number(a.order_number || Number.MAX_SAFE_INTEGER);
      const orderB = Number(b.order_number || Number.MAX_SAFE_INTEGER);
      if (orderA !== orderB) return orderA - orderB;
      return String(a.created_at || a.id).localeCompare(String(b.created_at || b.id));
    });

    return new Map(ordered.map((shipment, index) => [shipment.id, index + 1]));
  }, [shipments]);

  const tripOperationalCode = (shipment: Shipment) => {
    const sequence = tripSequenceById.get(shipment.id) || 0;
    return `V${trip?.trip_number || "—"}-${String(sequence).padStart(3, "0")}`;
  };
  ...
  const filteredShipments = shipments.filter((shipment) => {
    const matchesSearch =
      !normalizedSearch ||
      [
        tripSequenceById.get(shipment.id),
        tripOperationalCode(shipment),
  ...
  const selectedShipments = shipments
    ...
    .sort((a, b) => (tripSequenceById.get(a.id) || 0) - (tripSequenceById.get(b.id) || 0));
  ...
  async function changeOneStatus(shipment: Shipment, status: ShippingStatus) {
    ...
    const localNumber = tripSequenceById.get(shipment.id) || "—";
    if (!window.confirm(`¿Cambiar el envío #${localNumber} a “${getShippingStatusLabel(status)}”?`)) return;
    setWorking(true);
  ...
  function printSelectedManifest() {
    ...
    const rows = selectedShipments
      .map(
        (shipment) => `<tr>
          <td><strong>#${tripSequenceById.get(shipment.id) || "—"}</strong><br><small>${escapeHtml(tripOperationalCode(shipment))}</small></td>
          <td>${escapeHtml(shipment.tracking_code || shipment.id.slice(0, 8))}<br><small>Interno #${shipment.order_number || "—"}</small></td>
    ...
    <main className="min-h-screen bg-[#f5f7fb] p-4 pb-28 md:p-7">
      <div className="mx-auto max-w-[1500px]">
        ...
        <section className="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-black text-[#061b3a]">Envíos del viaje</h2><p className="text-sm font-semibold text-slate-500">La numeración visible comienza en 1 dentro de cada viaje. El consecutivo global se conserva internamente.</p></div>{!tripClosed && canManage && <Link href={`/admin/shipping/new?tripId=${encodeURIComponent(trip.id)}`} className="rounded-2xl bg-[#0a2d63] px-5 py-3 text-center font-black text-white">Registrar envío</Link>}</div>
