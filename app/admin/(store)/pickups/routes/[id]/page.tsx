...
import { ArrowDown, ArrowUp, CalendarDays, ExternalLink, Loader2, MapPin, MessageCircle, Save, Trash2, Truck } from "lucide-react";
...
export default function PickupRouteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    <header className="rounded-[2rem] bg-[#071d43] p-6 text-white sm:p-8">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><Link href="/admin/pickups/routes" className="text-sm font-black text-blue-200">← Volver a rutas</Link><h1 className="mt-3 text-3xl font-black sm:text-4xl">{route.name}</h1><p className="mt-2 text-blue-100/75">{route.route_date} · {route.stops?.length || 0} paradas · {cities.join(" · ") || "Sin ciudades"}</p></div><div className="flex flex-wrap gap-3"><select value={route.status} onChange={(e) => changeRouteStatus(e.target.value as PickupRouteStatus)} className="rounded-2xl bg-white px-4 py-3 font-black text-[#071d43]">{Object.entries(PICKUP_ROUTE_STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button onClick={saveInfo} disabled={saving} className="inline-flex items-center gap-2 rounded-2xl bg-red-600 px-5 py-3 font-black"><Save size={18} /> Guardar</button></div></div>
    </header>
