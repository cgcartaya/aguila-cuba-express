...
import { AlertTriangle, CalendarDays, CheckCircle2, Loader2, MapPin, Plus, Search, Truck } from "lucide-react";
...
export default function NewPickupRoutePage() {
  ...
  async function save() {
    if (!store?.id || !name.trim() || !routeDate) return setError("Escribe un nombre y selecciona una fecha para la ruta.");
  ...
  return <div className="grid gap-6 xl:grid-cols-[390px_1fr]">
    ...
    <section className="rounded-[2rem] border bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-black">Paradas disponibles</h2><p className="text-slate-500">Mezcla solicitudes de la web con clientes recibidos por WhatsApp.</p></div><div className="flex flex-col gap-2 sm:flex-row"><button onClick={() => setManualOpen(true)} className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-orange-500 px-5 font-black text-white shadow-sm"><Plus size={18} /> Agregar parada manual</button><label className="relative sm:w-72"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar..." className="h-12 w-full rounded-2xl border pl-11 pr-4 font-bold" /></label></div></div>
      <div className="mt-6 space-y-6">{Object.entries(grouped).map(([city, items]) => <div key={city}><div className="mb-3 flex items-center gap-2"><MapPin size={18} className="text-red-500" /><h3 className="text-lg font-black">{city}</h3><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-black">{items.length}</span></div><div className="grid gap-3 lg:grid-cols-2">{items.map((item) => {
        ...
        return <label key={item.id} className={`flex cursor-pointer gap-3 rounded-2xl border p-4 transition ${selectedNow ? "border-blue-500 bg-blue-50" : "border-slate-200"}`}><input type="checkbox" checked={selectedNow} onChange={() => toggle(item)} className="mt-1 h-5 w-5" /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-black">{item.customer_name} · {item.request_code}</p><SourceBadge source={item.request_source} /><Status mode={mode} /></div><p className="mt-1 text-sm font-bold text-slate-500">{item.address_line_1}, {item.postal_code}</p><p className="mt-2 flex items-center gap-1 text-xs font-black text-blue-700"><CalendarDays size={14} /> {(item.preferred_dates || []).join(" · ") || "Cualquier día"}</p>{mode === "incompatible" && selectedNow && <p className="mt-2 flex items-center gap-1 text-xs font-black text-amber-700"><AlertTriangle size={14} /> Pendiente de confirmar por WhatsApp</p>}</div></label>;
      })}</div></div>)}</div>
    </section>
    {store?.id && <ManualPickupStopModal open={manualOpen} storeId={store.id} routeDate={routeDate} onClose={() => setManualOpen(false)} onCreated={(item) => { setRequests((current) => [item, ...current]); setSelected((current) => current.includes(item.id) ? current : [...current, item.id]); }} />}
