...
export default function EconomyPage() {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      if (!mounted) return;
      setModuleEnabled(Boolean(settings?.module_economy_enabled));
      setChecking(false);
    }
    void check();
    return () => {
  ...
  if (accessLoading || storeLoading || checking) {
    return (
      <main className="min-h-screen bg-[#f4f7fb] p-6">
        <div className="mx-auto flex min-h-64 max-w-7xl items-center justify-center rounded-3xl bg-white">
    ...
    return (
      <main className="min-h-screen bg-[#f4f7fb] p-6">
        <div className="mx-auto max-w-4xl rounded-3xl bg-white p-10 text-center shadow-sm">
          Selecciona una tienda para consultar su economía.
        </div>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-[#f4f7fb] p-6">
        <div className="mx-auto max-w-4xl rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm font-semibold text-slate-500">
            Economía y rentabilidad no está activo para esta tienda. El Superadmin
            puede habilitarlo individualmente.
          </p>
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        {!moduleEnabled && isSuperAdmin && (
          <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800">
            Estás viendo este módulo como Superadmin. Para el dueño de la tienda
            continúa desactivado hasta que lo habilites en SaaS → Tiendas.
          </div>
        )}
