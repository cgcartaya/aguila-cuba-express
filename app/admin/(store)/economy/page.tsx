...
import EconomyPerformanceAnalytics from "@/components/admin/economy/EconomyPerformanceAnalytics";
...
export default function EconomyPage() {
  ...
  useEffect(()=>{let mounted=true;async function check(){if(accessLoading||storeLoading)return;if(!activeStore?.id){setChecking(false);return;}setChecking(true);const settings=await getEconomyModuleStatus(activeStore.id);if(!mounted)return;setModuleEnabled(Boolean(settings?.module_economy_enabled));setCurrency(settings?.economy_currency||"USD");setChecking(false);}void check();return()=>{mounted=false};},[accessLoading,storeLoading,activeStore?.id]);
  ...
  return <main className="min-h-screen bg-[#f4f7fb] p-4 pb-24 md:p-7"><div className="mx-auto max-w-[1500px]">
    <AdminPageHeader eyebrow="Control financiero" icon={CircleDollarSign} title="Economía y rentabilidad" description={`Costos, márgenes, gastos y rendimiento de ${activeStore.name||"la tienda activa"}.`}
      actions={<><Link href="/admin/economy/restaurante" className="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-4 py-2.5 text-sm font-black text-white shadow-sm"><UtensilsCrossed size={16}/>Costeo restaurante</Link><Link href="/admin/economy/compras" className="inline-flex items-center gap-2 rounded-xl bg-[#061b3a] px-4 py-2.5 text-sm font-black text-white shadow-sm"><PackagePlus size={16}/>Compras</Link><Link href="/admin/economy/proveedores" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-700"><Building2 size={16}/>Proveedores</Link></>}/>
    ...
    <EconomyPerformanceAnalytics storeId={activeStore.id} currency={currency}/>
  </div></main>;
}
