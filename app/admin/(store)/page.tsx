...
export default function AdminDashboardPage() {
  ...
  const width = 700, height = 190, padding = 18;
  const points = data.salesDays.map((day, index) => ({ x: padding + index * ((width - padding * 2) / 6), y: height - padding - day.total / maxSale * (height - padding * 2) }));
  ...
  return <main className="min-h-screen bg-[#f4f7fb] p-4 pb-24 md:p-7"><div className="mx-auto max-w-[1500px]">
    ...
    <section className="mt-5 grid gap-5 xl:grid-cols-[1.7fr_1fr]"><article className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm md:p-6"><div className="flex items-start justify-between"><div><h2 className="text-lg font-black text-[#061b3a]">Resumen de ventas</h2><p className="mt-1 text-xs font-semibold text-slate-400">Facturación diaria de los últimos 7 días</p></div><div className="text-right"><p className="text-xl font-black text-[#061b3a]">{money.format(weeklySales)}</p><p className="text-[10px] font-black uppercase text-slate-400">Total semanal</p></div></div><div className="mt-4 overflow-hidden"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Ventas de los últimos siete días" className="h-52 w-full"><defs><linearGradient id="salesArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#1769e8" stopOpacity=".26"/><stop offset="100%" stopColor="#1769e8" stopOpacity="0"/></linearGradient></defs>{[0.25,0.5,0.75].map((value) => <line key={value} x1={padding} x2={width-padding} y1={height*value} y2={height*value} stroke="#e2e8f0" strokeDasharray="5 6" />)}<polygon points={area} fill="url(#salesArea)"/><polyline points={line} fill="none" stroke="#1769e8" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/>{points.map((point, index) => <circle key={data.salesDays[index].key} cx={point.x} cy={point.y} r="5" fill="#fff" stroke="#1769e8" strokeWidth="3"><title>{data.salesDays[index].label}: {money.format(data.salesDays[index].total)}</title></circle>)}</svg><div className="grid grid-cols-7 text-center text-[10px] font-bold uppercase text-slate-400">{data.salesDays.map((day) => <span key={day.key}>{day.label}</span>)}</div></div></article>
